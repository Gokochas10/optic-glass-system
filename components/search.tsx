  ...
  useEffect(() => {
    if (query.length >= 3) {
      ...
      const filtered = results.filter(result => {
        if (type === "client") {
          // Buscar por nombre completo o RUC
          return (result.fullName && result.fullName.toLowerCase().includes(lowerCaseQuery)) ||
            (result.ruc && result.ruc.toLowerCase().includes(lowerCaseQuery));
        }
        return false;
      });
                ...
                >
                  {type === "client" ? (
                    <>
                      ...
                      <div className="text-xs text-gray-500">RUC: {result.ruc || 'No disponible'}</div>
                    </>
                  ) : (
                    <>
